  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < passungen.length; i++) {
      ...
      final shaftTop = spiel
          ? midY + h * 0.04       // Spielpassung: Welle kleiner als Bohrung
          : i == 1
              ? midY - h * 0.22   // Übergang: fast gleich
      ...
      paint
        ...
        ..style = PaintingStyle.fill;
      canvas.drawRect(Rect.fromLTRB(x + 4, shaftTop, x + colW - 4, shaftBot), paint);
      paint
        ..color = cs.secondary
        ..strokeWidth = 1.5
        ..style = PaintingStyle.stroke;
      ...
      // Beschriftung
      _text(canvas, titel, Offset(x + 2, h * 0.03),
          TextStyle(color: cs.onSurface, fontSize: 9, fontWeight: FontWeight.w600), cs.onSurface);
      _text(canvas, code, Offset(x + 2, h * 0.88),
          TextStyle(color: cs.outline, fontSize: 9), cs.outline);
    }

    // Legende
    _textBox(canvas, '  Bohrung (H)', Offset(w * 0.05, h * 0.92), cs.primary);
    _textBox(canvas, '  Welle (Passungsart)', Offset(w * 0.35, h * 0.92), cs.secondary);
  }
