...
void main() {
  ...
  test('LernplanLoader lädt alle 5 Themenbereiche aus den JSON-Dateien', () async {
    final lernplan = await LernplanLoader().laden();

    expect(lernplan.themenbereiche.length, 5);

    final fertigungstechnik = lernplan.themenbereiche
        .firstWhere((tb) => tb.id == 'fertigungstechnik');
    ...
    final platzhalterBereiche = lernplan.themenbereiche
        .where((tb) => tb.id != 'fertigungstechnik');
